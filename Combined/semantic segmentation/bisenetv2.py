...
class SegmentHead(nn.Module):
    ...
    def __init__(self, in_chan, mid_chan, n_classes, up_factor=8, aux=True):
        ...
        if aux:
            self.conv_out = nn.Sequential(ConvBNReLU(mid_chan, up_factor * up_factor, 3, stride=1), nn.Conv2d(up_factor * up_factor, out_chan, 1, 1, 0), nn.PixelShuffle(up_factor))
        else:
            self.conv_out = nn.Sequential(nn.Conv2d(mid_chan, out_chan, 1, 1, 0), nn.PixelShuffle(up_factor))
...
class BiSeNetV2(nn.Module):
    ...
    def __init__(self, n_classes, output_aux=False, return_logits=False):
    ...
    def forward(self, x):
        ...
        return logits if self.return_logits else logits.argmax(dim=1)
